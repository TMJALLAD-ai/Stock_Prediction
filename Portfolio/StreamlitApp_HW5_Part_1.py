...
MODEL_INFO = {
        ...
        "keys": ["IBM"],
        "inputs": [{"name": "IBM", "type": "number", "min": 0.0, "max": 500.0, "default": 100.0, "step": 10.0}]
}
...
def display_explanation(user_inputs, session, aws_bucket):
    ...
    lookup_stock = 'IBM'
    lookup_price = user_inputs['IBM']
    closest_date = (dataset[lookup_stock] - float(lookup_price)).abs().idxmin()
    ...
    # Create preprocessing pipeline (all steps except model)
